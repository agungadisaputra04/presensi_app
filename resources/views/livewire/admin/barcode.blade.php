<div class="p-6 lg:p-8">
  ...
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
    ...
    <div
      ...
      <div id="qrcode{{ $barcode->id }}" class="h-64 w-64 bg-transparent"></div>
      </div>
  ...
  @empty
  <p class="text-gray-500 dark:text-gray-300">Tidak ada data barcode.</p>
@endforelse
  </div>
...
@if ($barcodes->count())
  <script type="text/javascript">
    const barcodes = @json($barcodes->map(fn($b) => [
    'id' => $b->id,
    'value' => $b->value,
  ]));
    ...
    barcodes.forEach(barcode => {
      const el = document.getElementById("qrcode" + barcode.id);
      if (el) {
