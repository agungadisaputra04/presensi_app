    ...
    <div class="flex h-16 justify-between">
      <div class="flex">
        ...
        <div class="flex shrink-0 items-center">
          <a href="{{ Auth::user()->isAdmin ? route('admin.dashboard') : route('home') }}">
            <img src="{{ asset('assets/agung/logo-S247.png') }}" alt="Logo" class="block h-9 w-auto" />
          </a>
        </div>
        ...
        <div class="hidden space-x-2 sm:-my-px sm:ms-6 sm:flex md:ms-10 md:space-x-5 lg:space-x-8">
          ...
          <x-nav-dropdown :active="request()->routeIs('admin.masters.*')" triggerClasses="text-nowrap">
            ...
            <x-slot name="content">
              ...
              <x-dropdown-link href="{{ route('admin.masters.schedule') }}" :active="request()->routeIs('admin.masters.schedule')">
                {{ __('Scheduler') }}
            </x-dropdown-link>
      ...
      <div class="flex gap-2">
        <div class="hidden sm:ms-6 sm:flex sm:items-center">
          <x-theme-toggle />
          <div class="relative">
            <!-- Komponen Livewire untuk notifikasi -->
            @livewire('navbar-notification', ['pollingInterval' => 5]) 
        </div>
...
<div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
  <div class="space-y-1 pb-3 pt-2">
    ...
    <x-responsive-nav-link  href="{{ route('admin.masters.schedule') }}" :active="request()->routeIs('admin.masters.schedule')">
      {{ __('Scheduler') }}
  </x-responsive-nav-link>
